...
@UIScoped
public class ValoMainMenuBean extends CssLayout implements MainMenu {
    ...
    private void onMenuItemClicked(String navigationResource) {
        navigationEventSource.fire(new NavigationEvent(navigationResource, null, EventType.PROGRAMMATIC));
    }
    ...
    protected void onNavigationEvent(@Observes(notifyObserver = Reception.IF_EXISTS) NavigationEvent e) {
        ...
        Optional<MainMenuItemBean> selectedMenuItem = menuItems.stream()
                .filter(menuItem -> menuItem.getViewName().equals(e.getView())).findFirst();
